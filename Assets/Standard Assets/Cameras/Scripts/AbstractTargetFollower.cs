...
{
    public abstract class AbstractTargetFollower : Bolt.EntityEventListener<IAvatarState>
    {
        ...
        {
            if (state.AvatarName != "" && entity.isOwner)
            {
                // auto target an object tagged player, if no target has been assigned
                var targetObj = GameObject.FindGameObjectWithTag("Player");
                string playername = targetObj.GetComponent<TextMesh>().text;
                if (targetObj && playername == state.AvatarName)
                {
